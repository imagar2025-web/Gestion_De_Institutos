    ...
    <div class="card">
      ...
      <form @submit.prevent="modoEdicion ? actualizarEspacio() : insertarEspacio()">
        ...
        <div class="form-group">
          ...
          <!-- Los valores están hardcodeados porque estado_operativo no tiene tabla en BD.
               Son constantes del dominio definidas en el frontend. -->
      ...
      <div v-else-if="espacios.length === 0" class="vacio">No hay espacios registrados</div>
      <div v-else class="tabla-wrapper">
        <table>
          <thead>
            <tr>
              <th>ID</th><th>Nombre</th><th>Planta</th><th>Capacidad</th>
              <th>Equipamiento</th><th>Estado</th><th>Acciones</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="e in espacios" :key="e.id"
              ...
              <td>
                <!-- El badge aplica la clase dinámica 'estado-operativa', 
                     'estado-mantenimiento' o 'estado-fuera_de_servicio' -->
...
<script setup>
// ─────────────────────────────────────────────────────────────
// CrearEspacios.vue  —  CRUD completo de Espacios
// Gestiona las aulas, laboratorios y otros espacios del centro.
// El campo estado_operativo NO proviene de una tabla BD; es una
// constante del dominio con 3 valores fijos: operativa, 
// mantenimiento, fuera_de_servicio.
// Solo los espacios con estado_operativo = 'operativa' aparecen
// disponibles en ReservarEspacio.vue.
// ─────────────────────────────────────────────────────────────
...
const espacios    = ref([]);   // Array reactivo que alimenta la tabla
const mensaje     = ref("");
const mensajeError = ref(false);
const cargando    = ref(false);
const modoEdicion = ref(false);

// Función factoría: objeto espacio vacío para inicializar/resetear el formulario
const espacioVacio = () => ({
  id:               "",
  nombre:           "",
  ubicacion_planta: "",
  capacidad_max:    "",
  equipamiento:     "",
  estado_operativo: "",  // Debe ser uno de: operativa | mantenimiento | fuera_de_servicio
...
// Carga la lista de espacios al montar el componente
onMounted(() => cargarEspacios());

// GET /espacios → rellena la tabla
const cargarEspacios = async () => {
  ...
  try {
    ...
    cargando.value = false;  // Siempre se apaga el spinner
  }
};
...
const insertarEspacio = async () => {
  try {
    ...
    espacio.value = espacioVacio();  // Resetea el formulario
    await cargarEspacios();          // Refresca la tabla
  } catch (error) {
    if (error.response?.data?.error?.includes("duplicate key")) {
      // Error específico de clave duplicada en la BD
...
// Carga los datos del espacio seleccionado en el formulario para editarlos
const cargarEnFormulario = (e) => {
  // Spread copia todos los campos; luego se sobreescriben los de auditoría
...
// PUT → actualiza el espacio editado
...
// DELETE → elimina el espacio tras confirmación
const eliminarEspacio = async (id) => {
  ...
  try {
    ...
    // Si se borra el que estaba en edición, cancela el modo edición
